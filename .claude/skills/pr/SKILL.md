...
- [x] `poetry run pytest` — N passed
- [x] `poetry run pre-commit run --all-files` — passed
- [x] `cd frontend && npm test -- --watchAll=false` — N passed
...
to look out for (e.g. "you should see X in the browser", "check that Y
does NOT appear", "the API should return 200"). The reviewer should be
...
**When to include "Human verification"**: Always include when there are unchecked manual test items. Also include when the PR contains decisions that could affect data integrity, security, or API contracts. Omit only for routine changes where all tests are automated and pass.
...
1. **Guard**: If the current branch name starts with `worktree-agent-`, STOP. Create a properly named branch (e.g. `feat/...`, `fix/...`) from the current HEAD first. Never push `worktree-agent-*` branches to remote.
2. Examine all commits on the current branch vs the base branch (default: `dev`, or `$ARGUMENTS` if provided)
3. Draft a short PR title (under 70 chars) and body following the format above
4. Present the draft and ask the user to review before creating the PR
5. Once approved, push and create the PR using `gh pr create`
