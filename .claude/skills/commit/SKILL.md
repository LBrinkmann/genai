...
4. **Run `git diff --staged` and review every hunk for non-functional changes.** Revert any hunks that only reformat, re-wrap, rewrite docstrings/comments, rename variables for style, or remove documentation on unchanged code. Only functional changes belong in the commit.
5. Show the user the list of files that will be committed and get confirmation before proceeding
